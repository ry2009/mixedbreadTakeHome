...
This repository contains a disk-backed multivector store tailored for late-interaction models (ColBERT, ColPali, etc.).
The implementation focuses on **cold-start latency** and **memory efficiency** while keeping the API minimal: `insert`, `delete`, and `search`.

## Highlights
- Persistent storage on NVMe SSDs using a compact binary layout (`vectors.bin` + `index.pkl`).
- Zero-copy reads via `os.pread` and NumPy buffer views; no need to memory-map the full index.
- Scoring backends selectable at runtime: [`maxsim-cpu`](https://github.com/mixedbread-ai/maxsim-cpu) for peak latency or a pure NumPy fallback when the native kernel is unavailable.
- Optional threadpool parallelism for NumPy scoring accelerates CPU-heavy workloads when native kernels are unavailable.
- CLI tooling for building synthetic datasets and running reproducible benchmarks.
- Automatic fragmentation management: once at least 64 MiB of garbage accumulates and exceeds 35 % of on-disk bytes the store triggers compaction in-place. Manual compaction is still exposed via CLI/API.
...
The CLI mirrors the data generation script from the prompt and supports batching plus repeatable seeds.
...
- Vectors are generated as `int8` with per-document sequence lengths sampled uniformly between 10 and 250 tokens.
...
The benchmark script samples `candidates` document IDs per query, generates a `(32, 128)` float32 query, and reports:
- `Cold start latency` (first call, cache-cold)
- `Warm avg latency` across the remaining calls
- `Warm p95 latency`

### Backend selection
The store automatically uses `maxsim-cpu` when it’s importable. To force the NumPy fallback (for benchmarks or constrained environments), set `MVSTORE_DISABLE_MAXSIM=1` or instantiate `DiskBackedMultiVectorStore(..., backend="numpy")`. Threadpool parallelism for the NumPy path can be tuned via `max_workers` (defaults to roughly half the visible hardware threads; pass 1 to disable).

### Compact the store
After large batches of deletions, run compaction to reclaim SSD space:

```bash
mvstore-benchmark --store-dir ./data compact
```

The command rewrites `vectors.bin` in place and persists the refreshed index.
...
For multi-tenant deployments instantiate your own `DiskBackedMultiVectorStore` pointing to an isolated directory.
...
```
This mirrors the intended production flow: Stage 1 narrows millions of documents to ~1 000 candidates, Stage 2 streams their token embeddings off NVMe, runs MaxSim against the query, and returns a ranked list while holding only the candidate set in memory.

## Benchmarks
All timings captured on the local development machine (`Darwin 24.6.0, 8-core Intel`) using Python 3.11 and the CLI commands shown above.
...
Notes:
- Cold-start test executed immediately after store construction (macOS lacks `drop_caches`).
- Each search touches 1,000 candidate documents and performs NumPy MaxSim scoring against a `(32, 128)` query.
- Warm calls exhibit stable latency thanks to the OS page cache and contiguous on-disk layout.
- Docker Desktop numbers are conservative due to overlayfs and virtualised storage; expect materially lower latency on the target `c4-standard-8-lssd` once benchmarks are run there with `echo 3 > /proc/sys/vm/drop_caches`.
- TODO: capture official `c4-standard-8-lssd` metrics when the cloud instance is available.

## Design Notes
- **Data layout**: Each record stores `(doc_id:uint64, seq_len:uint32, dim:uint32, int8 vectors...)`. Append-only writes plus a compact pickled index keep persistence simple and S3-friendly.
- **Reads**: `os.pread` streams records without disturbing the file pointer, allowing concurrent search threads. NumPy views avoid copies when reshaping the int8 payload.
- **MaxSim**: Queries are converted to float32 once per call; vectors are row-normalised before BLAS multiplication so NumPy and `maxsim-cpu` return identical scores.
- **Deletes / updates**: Deletions and reinserts mark old segments as garbage. The store tracks tombstone bytes and auto-compacts when fragmentation crosses configurable thresholds.
- **Parallel fallback**: When native kernels are unavailable the NumPy path fans out across a threadpool (defaults to half the hardware threads) so the BLAS-backed matmuls can run concurrently.
...
## Possible Extensions
1. Parallelize I/O and scoring across multiple worker threads when `maxsim-cpu` is present.
2. Expose fragmentation metrics via Prometheus to monitor compaction frequency across tenants.
3. Extend the index to store auxiliary metadata (e.g., tenant IDs, document lengths) for smarter candidate pruning prior to MaxSim scoring.

## Testing
- `python -m compileall src` ensures the package imports cleanly.
- `mvstore-benchmark` CLI used to validate end-to-end insertion, search, compaction, and latency benchmarks.
- Docker Desktop quick check (Linux VM, overlayfs storage):
  ```bash
  ...
  ```
  (Outputs the 860 ms / 624 ms / 866 ms figures shown above.)

## Optional acceleration
The store will automatically use `maxsim-cpu` if the Python bindings are importable. Recommended installation on Linux:

```bash
python -m pip install git+https://github.com/mixedbread-ai/maxsim-cpu
```

Quick verification (should print identical score lists twice):
